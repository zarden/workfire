...
void DownAdjust(Heap *hp, int n)
{
	...
	{
		if (hp->_data[cur] < hp->_data[cur * 2 + 1])//大根堆中，如果我比左孩子小。就要执行交换
		{
			if (cur * 2 + 2 >= hp->_size ||//如果右孩子不存在，直接交换
				hp->_data[cur * 2 + 1] >= hp->_data[cur * 2 + 2])//如果左孩子比右孩子大，那么我就跟左孩子换
			{
				swap(&hp->_data[cur], &hp->_data[cur * 2 + 1]);//换值
				cur = cur * 2 + 1;//换位
			}
			else//如果右孩子存在且比左孩子大，那么跟右孩子换
			{
				swap(&hp->_data[cur], &hp->_data[cur * 2 + 2]);
				cur = cur * 2 + 2;
			}
		}
		else if (cur * 2 + 2 < hp->_size&&hp->_data[cur] < hp->_data[cur * 2 + 2])//如果比左孩子大，比右孩子小，那么跟右孩子交换
		{
			swap(&hp->_data[cur], &hp->_data[cur * 2 + 2]);
			cur = cur * 2 + 2;
		}
